...
int TagBoundaryLayer (Options *mshopt)
{
	Mesh *Msh = NULL;
	char OutSol[1024];
	...
	if ( mshopt->DisMax < 0 ) {
		printf(" ## ERROR : a max distance to wall must be prescribed using the -dis command line option.\n");
		return 0;
	}
	

	BoundaryLayerTag(mshopt, Msh);
